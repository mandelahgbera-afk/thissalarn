...
export default function CopyTrading() {
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      await api.balances.update(user.email, { balance_usd: bal.balance_usd - allocAmt });
  ...
  const handleStop = async (copy: any) => {
    try {
      ...
      const refund = (copy.allocation ?? 0) + (copy.profit_loss ?? 0);
      if (refund > 0) {
        const bal = await api.balances.getByEmail(user!.email);
        const currentBal = bal?.balance_usd ?? 0;
        await api.balances.update(user!.email, { balance_usd: currentBal + refund });
